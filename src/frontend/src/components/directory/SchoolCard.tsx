...
import { getCuratedThumbnail } from '@/lib/curatedSchoolMedia';
...
export default function SchoolCard({ school }: SchoolCardProps) {
  ...
  // Thumbnail selection: prefer curated thumbnail, fall back to deterministic thumbnail
  const curatedThumb = getCuratedThumbnail(school.id);
  const fallbackThumbnail = getSchoolThumbnail(school.id);
  const thumbnailSrc = curatedThumb || fallbackThumbnail;
